...
export const handleSubscribe = async (priceId: string, clinicId: any, successUrl?: string) => {
  if (!clinicId) return;
  ...
  try {
    const res = await fetch(`${process.env.NEXT_PUBLIC_SUPABASE_URL}/functions/v1/create-checkout-session`, {
      ...
      body: JSON.stringify({ clinic_id: clinicId, price_id: priceId, success_url: successUrl }),
    });
