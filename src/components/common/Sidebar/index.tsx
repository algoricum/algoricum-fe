      ...
      <Flex vertical className="h-full">
        ...
        {(!path.includes("content") || !path.includes("settings")) && (
          <Flex
            justify="center"
            align="center"
            className={`bg-Black100 rounded-full absolute top-[55px] right-0 cursor-pointer ${isCollapsed && "rotate-180"}`}
        ...
        <Flex vertical justify="space-between" className="flex-1">
          ...
          <div className="flex flex-col justify-end items-center gap-2">
            {footerItems.map(({ key, label, icon: Icon, selectedIcon }, index) => (
              <div
                key={index}
                className={`w-full flex flex-row items-center py-2 gap-2 cursor-pointer border-b-2 border-transparent !border  hover:border-Primary900 rounded hover:!bg-Primary50  ${
                  isSelected(key) && "!bg-Primary50 !rounded !border-Primary900"
