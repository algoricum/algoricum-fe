...
function getRequiredDNSRecords(subdomain: string, logger: Logger) {
  ...
  const records = {
    ...
    txt_records: [{ type: "TXT", host: subdomainHost, value: "v=spf1 include:mailgun.org ~all" }],
    cname_records: [{ type: "CNAME", host: `email.${subdomainHost}`, value: "mailgun.org" }],
  };
...
async function handleSetupAction(
  ...
  if (!slugValidation.valid) {
    ...
    if (fixedValidation.valid) {
      ...
      await supabase.from("clinic").update({ slug: clinicSlug, updated_at: new Date().toISOString() }).eq("id", clinic.id);
    } else {
  ...
  const { error: clinicUpdateError } = await supabase.from("clinic").update(clinicUpdateData).eq("id", clinic.id);
  ...
  const { error: settingsError } = await supabase.from("mailgun_settings").upsert(settingsData);
...
async function handleDeleteAction(clinic: any, mailgunApiKey: string, supabase: any, logger: Logger, requestStart: number) {
  ...
  if (clinic.domain) {
    ...
    try {
      const deleteResponse = await fetch(`https://api.mailgun.net/v3/domains/${clinic.domain}`, {
        method: "DELETE",
        headers: {
          Authorization: `Basic ${btoa(`api:${mailgunApiKey}`)}`,
        },
    ...
    try {
      const { error: settingsDeleteError } = await supabase.from("mailgun_settings").delete().eq("clinic_id", clinic.id);
