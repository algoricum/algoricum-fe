// supabase/functions/facebook-lead-form/index.ts
// Supabase Edge Function (Deno + TypeScript)
// Routes:
//  GET  /auth/start?clinic_id=<uuid>&redirect_to=<optional_url>
//  GET  /auth/callback?code=...&state=...    (facebook oauth callback)
//  POST /fetch-facebook-lead-form-responses  (body: { clinic_id })
...
const ORIGIN_OVERRIDE = Deno.env.get("ORIGIN") || null;

if (!SUPABASE_URL || !SUPABASE_SERVICE_ROLE_KEY || !FACEBOOK_APP_ID || !FACEBOOK_APP_SECRET) {
  console.error(
    "Missing required environment variables. Ensure SUPABASE_URL, SUPABASE_SERVICE_ROLE_KEY, FACEBOOK_APP_ID and FACEBOOK_APP_SECRET are set.",
  );
}
...
serve(async req => {
  ...
  try {
    ...
    if (path.endsWith("/auth/callback") && method === "GET") return await handleAuthCallback(req, url, supabaseAdmin);
    ...
    if (path.endsWith("/facebook-webhook") && method === "POST") return await handleFacebookWebhook(req, supabaseAdmin);
...
/* ------------------------------
   Helper: auth start
   ------------------------------ */
async function handleAuthStart(req: Request, url: URL) {
...
/* ------------------------------
   Helper: auth callback
   Exchanges code -> short-lived token -> long-lived token
   Fetches pages (with page access tokens) and lead forms; upserts connections; subscribes to page
   ------------------------------ */
async function handleAuthCallback(req: Request, url: URL, supabaseAdmin: any) {
...
/* ------------------------------
   Fetch historical leads (POST /fetch-facebook-lead-form-responses)
   body: { clinic_id }
   ------------------------------ */
async function fetchFacebookLeadFormResponses(clinic_id: String, supabaseAdmin: any) {
  try {
    if (!clinic_id)
      return new Response(JSON.stringify({ error: "clinic_id is required" }), {
        status: 400,
...
/* ------------------------------
   Webhook verify (GET) & process (POST)
   ------------------------------ */
