...
export async function discoverPipedriveProperties(
  accessToken: string, 
  apiDomain: string, 
  requestId: string
) {
...
export async function syncAllLeads(modifiedSince?: string): Promise<{ success: boolean; results: any[]; total_synced: number; error?: string }> {
  const requestId = crypto.randomUUID()
