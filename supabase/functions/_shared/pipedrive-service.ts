...
export async function handleOAuthCallback(
  ...
  try {
    ...
    if (state && state.includes("|")) {
      const [parsedClinicId, , parsedRedirectUrl] = state.split("|");
...
export async function syncPipedriveIntegration(
  ...
  if (leadsToInsert.length > 0) {
    try {
      // Clean leads with GPT before enqueuing
      console.log(`[${requestId}] Cleaning ${leadsToInsert.length} Pipedrive leads with GPT`);
      const cleanedLeads = await extractAndCleanContacts(leadsToInsert, requestId);

      console.log(`[${requestId}] GPT cleaned ${cleanedLeads.length} leads from ${leadsToInsert.length} raw leads`);

      if (cleanedLeads.length > 0) {
        // Use the queue system to process leads
        const chunks = chunkArray(cleanedLeads, 50); // Process in chunks of 50
        console.log(`[${requestId}] Enqueuing ${cleanedLeads.length} leads in ${chunks.length} chunks for clinic ${integration.clinic_id}`);

        for (const chunk of chunks) {
          await enqueueLead(chunk, integration.clinic_id);
        }

        insertedCount = cleanedLeads.length;
        console.log(`[${requestId}] Successfully enqueued ${insertedCount} leads for clinic ${integration.clinic_id}`);
      } else {
        console.warn(`[${requestId}] No valid leads after GPT cleaning for clinic ${integration.clinic_id}`);
      }
    } catch (error) {
      console.error(`[${requestId}] Error processing leads for clinic ${integration.clinic_id}:`, error);
      throw new Error(`Failed to process leads: ${error.message}`);
    }
  }
...
export type { PipedriveIntegration, PipedriveLead, PipedrivePerson, MappedLead, SyncResult };
