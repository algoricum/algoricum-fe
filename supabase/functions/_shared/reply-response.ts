...
export async function generateAIResponse(
  ...
  try {
    ...
    const isBookingInquiry = options.messageBody.toLowerCase().includes('book') || 
...
RESPONSE FORMAT: SMS response - Keep main message under 160 characters, be conversational and helpful, NO links or unsubscribe options.`.trim()
    ...
    while ((runStatus.status === 'in_progress' || runStatus.status === 'queued') && attempts < maxAttempts) {
      ...
      try {
        ...
        logError('❌ CRITICAL: Run status retrieval failed during retry, falling back to Chat Completions instead of using Assistant with data')
        return await generateFallbackResponse(leadData, options, clinicData, conversationContext, openaiApiKey, isEmailResponse)
      }
    ...
    if (runStatus.status === 'completed') {
      ...
      logError('❌ CRITICAL: Assistant run incomplete (file search timeout), falling back to Chat Completions instead of using Assistant with data')
      return await generateFallbackResponse(leadData, options, clinicData, conversationContext, openaiApiKey, isEmailResponse)
    } else {
