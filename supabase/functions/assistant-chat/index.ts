      ...
      if (error.message.includes('Path parameters result in path with invalid segments')) {
        ...
        try {
          const response = await fetch(
            `https://api.openai.com/v1/threads/${threadIdStr}/runs/${runIdStr}`,
            {
              method: 'GET',
              headers: {
...
serve(async (req) => {
  ...
  try {
    ...
    const [assistantData, threadData] = await Promise.all([
      ...
      supabaseClient.from('threads').select('lead_id, lead:lead_id(first_name, last_name, email, phone, status, interest_level, urgency)').eq('id', thread_id).single()
    ]);
    ...
    console.log('📤 Sending final response - Clean message length:', cleanMessage.length, '| Assessment valid:', assessmentData.hasValidAssessment);
    return new Response(JSON.stringify({
