...
serve(async (req) => {
  ...
  if (req.method === "GET" && path.endsWith("/auth/start")) {
    ...
    const authUrl =
      `https://api.typeform.com/oauth/authorize?client_id=${TYPEFORM_CLIENT_ID}&redirect_uri=${redirectTo}&scope=responses:read forms:read webhooks:write webhooks:read&state=${clinicId}`;
  ...
  if (req.method === "GET" && path.endsWith("/auth/callback")) {
    ...
    return Response.redirect(`${REDIRECT_URI}?typeform_status=success`, 302);
  }

  // 🔹 STEP 3: Update Forms (now also fetches old leads for new forms)
          ...
          if (res.ok) {
            ...
            for (const r of respData.items || []) {
              ...
              await supabase.from("lead").insert({
                clinic_id,
                source_id: "bf1bb50b-d6dd-4c11-ba96-2f7aac74895c", // must map to lead_source FK
  ...
  // 🔹 STEP 4: Webhook (new submissions)
  if (req.method === "POST" && path.endsWith("/webhook")) {
    ...
    // 🔹 Insert into correct "lead" table
...
if(webhok.error) {
  ...
  // 🔹 STEP 5: Get Forms (like getSheets)
