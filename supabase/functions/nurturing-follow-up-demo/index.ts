...
const { info: logInfo, error: logError } = createLogger("DEMO_FOLLOWUPS");
...
serve(async req => {
  ...
  try {
    ...
    // Process demo nurturing follow-ups using shared service with demo_only filter
    logInfo("Starting demo nurturing follow-ups processing");
    const result = await processScheduledFollowUps(supabase, "demo_only");
